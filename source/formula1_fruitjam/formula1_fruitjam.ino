...
void loop()
{
    ...
    if(gamepadButtonJustPressed(GAMEPAD_LEFT_SHOULDER) || keyJustPressed(F1KEY) ||
		((buttons & BUTTON_2_MASK) && (buttons & BUTTON_1_MASK) && ! (prevbuttons & BUTTON_1_MASK)))
		decVolumeSound();

	if(gamepadButtonJustPressed(GAMEPAD_RIGHT_SHOULDER) || keyJustPressed(F2KEY) ||
		((buttons & BUTTON_2_MASK) && (buttons & BUTTON_3_MASK) && ! (prevbuttons & BUTTON_3_MASK)))
		incVolumeSound();
