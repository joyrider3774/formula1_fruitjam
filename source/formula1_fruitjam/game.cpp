...
void DrawGame(void)
{
	...
	for (X = 0; X < 3; X++)
		for (Y = 0; Y < 3; Y++)
			if (EnemyStates[X][Y])
			{
				bufferDrawImage(&fb,90 + (X * 54), 48 + (Y * 47), enemy_data,enemy_width,enemy_height,false,true,false);
			}
	...
	for (X = 0; X < 3; X++)
	{
		if (PlayerStates[X])
		{
			bufferDrawImage(&fb,90 + (X * 54), 180, player_data, player_width, player_height,false,true,false);
		}
	}
}
...
void game(void)
{
    ...
    if (gamepadButtonJustPressed(GAMEPAD_RIGHT) || (buttons & BUTTON_1_MASK) && !(prevbuttons & BUTTON_1_MASK) || 
		mouseButtonJustPressed(1) || keyJustPressed(RIGHTKEY) || keyJustPressed(BUTTON1_KEY))
    {
		if (CanMove)
		{
			MoveRight();
    ...
    if (gamepadButtonJustPressed(GAMEPAD_LEFT) || (buttons & BUTTON_3_MASK) && !(prevbuttons & BUTTON_3_MASK) || 
		mouseButtonJustPressed(1) || keyJustPressed(LEFTKEY) || keyJustPressed(BUTTON3_KEY))
    {
		if (CanMove)
		{
			MoveLeft();
...
void gameOver(void)
{
	...
	if (gamepadButtonJustPressed(GAMEPAD_RIGHT) || gamepadButtonJustPressed(GAMEPAD_LEFT) || 
		...
		(buttons & BUTTON_3_MASK) && !(prevbuttons & BUTTON_1_MASK) ||
...
{
	uint8_t X;
	bufferFillRect(&fb,0,0,tft.width(),tft.height(), 0x0000);
	...
	if (gamepadButtonJustPressed(GAMEPAD_A) || gamepadButtonJustPressed(GAMEPAD_B) || 
        (buttons & BUTTON_1_MASK) && !(prevbuttons & BUTTON_1_MASK) || 
