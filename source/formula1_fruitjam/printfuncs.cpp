...
void printNumber(const uint8_t fontBitmapTable[], int32_t ax, int32_t ay, uint16_t tileSize, int32_t aNumber, int32_t maxDigits)
{
    ...
    for (int32_t c = 0; c < maxFor; c++)
    {
        ...
        bufferDrawImage(&fb, (ax + (maxDigits - digits) + c) * tileSize, ay * tileSize, fontBitmapTable+(((number[buffSize - digits + c] - 47) * tileSize*tileSize*sizeof(uint16_t))), tileSize,tileSize,false,true,false);
...
void printMessage(const uint8_t fontBitmapTable[], int32_t ax, int32_t ay, uint16_t tileSize, const char* amsg)
{
    ...
    while (1)
    {
        ...
        bufferDrawImage(&fb,(ax + index) * tileSize, ay * tileSize, fontBitmapTable+(tile*tileSize*tileSize*sizeof(uint16_t)), tileSize, tileSize,false,true,false);
        ++index;
    }
}
